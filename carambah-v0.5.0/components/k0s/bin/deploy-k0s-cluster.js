...
const ConfigUtil = require('../core/lib/fs/config');
const DockerUtil = require('../core/lib/docker/docker');
const KubectlUtil = require('../core/lib/kubectl/kubectl');
const ExecUtil = require('../core/lib/exec');
...
class K0sClusterDeployment {
    ...
    async initialize() {
        try {
            this.logger.header('InfoMetis v0.5.0 - k0s Cluster Deployment', 'JavaScript Native Implementation');
    ...
    /**
     * Ensure k0s image is available in Docker
     */
    async ensureK0sImage() {
        this.logger.step('Ensuring k0s image is available...');
        
        const k0sImage = 'k0sproject/k0s:latest';
        
        // Check if image exists in Docker
        const imageExists = await this.exec.run(`docker inspect "${k0sImage}"`, {}, true);
        if (imageExists.success) {
            this.logger.success('✓ k0s image already available in Docker');
            return true;
        }
        
        // Image not in Docker cache - will be pulled during container creation
        this.logger.info('ℹ️ k0s image not in Docker cache, will be pulled from registry');
        return true;
    }
