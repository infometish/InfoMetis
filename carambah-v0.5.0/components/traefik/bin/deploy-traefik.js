...
const DockerUtil = require('../core/docker/docker');
...
class TraefikDeployment {
    ...
    async isImageInContainerd(image) {
        try {
            ...
            const patterns = [
                image,                                    // exact match: traefik:v2.9
                `library/${image}`,                      // docker.io/library/traefik:v2.9
                image.replace(':', ' ')                  // space-separated: traefik v2.9
            ];
    ...
    /**
     * Get all images required for Traefik deployment
     */
    getRequiredImages() {
        const traefikImages = this.imageConfig.images.filter(img => 
            img.includes('traefik') ||
            img.includes('kube-router') ||
            img.includes('coredns') ||
            img.includes('apiserver-network-proxy') ||
            img.includes('cni-node') ||
            img.includes('kube-proxy') ||
            img.includes('metrics-server') ||
            img.includes('pause')
        );
        return traefikImages;
    }

    /**
     * Ensure all required images are available in k0s containerd
     */
    async ensureImagesAvailable() {
        this.logger.step('Ensuring Traefik and k0s system images are available in k0s containerd...');
        
        const requiredImages = this.getRequiredImages();
        ...
        this.logger.success(`All ${requiredImages.length} required images are now available in k0s containerd`);
        return true;
    }
    ...
    async deployTraefikComplete() {
        ...
        try {
            ...
            const manifestPath = path.join(__dirname, '..', 'environments', 'kubernetes', 'manifests', 'traefik-deployment.yaml');
