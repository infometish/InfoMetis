...
NAMESPACE="infometis"
NIFI_MANIFEST="/workspace/config/manifests/nifi-k8s.yaml"
...
docker exec infometis sh -c "cd /workspace && k0s kubectl apply -f config/manifests/nifi-k8s.yaml" || {
    echo "❌ Failed to apply NiFi manifest"
    exit 1
}

echo "📋 Deploying NiFi ingress..."
docker exec infometis sh -c "cd /workspace && k0s kubectl apply -f config/manifests/nifi-ingress.yaml" || {
