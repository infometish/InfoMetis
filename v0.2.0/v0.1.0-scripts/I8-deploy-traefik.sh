...
if ! docker exec infometis test -f "/workspace/config/manifests/traefik-deployment.yaml"; then
    echo "❌ Traefik manifest not found in container: /workspace/config/manifests/traefik-deployment.yaml"
    exit 1
fi
echo "✅ Traefik manifest found"

echo "📋 Deploying Traefik resources..."
docker exec infometis sh -c "cd /workspace && k0s kubectl apply -f config/manifests/traefik-deployment.yaml" || {
