...
class InteractiveConsole {
    constructor() {
        ...
        this.running = true;
        this.lastExecutedFunction = null;
        this.lastExecutedSection = null;
    ...
    displayMainMenu() {
        ...
        if (this.consoleConfig && this.consoleConfig.sections) {
            this.consoleConfig.sections.forEach((section, index) => {
                const sectionLine = `${section.icon} ${section.key.toUpperCase()}: ${section.name}`;
                if (this.lastExecutedSection === section.key) {
                    console.log(this.logger.bold(sectionLine));
                } else {
        ...
        console.log('   a       - Auto execute all sections');
    ...
    displaySectionMenu(section) {
        ...
        if (section.steps) {
            section.steps.forEach((step, index) => {
                ...
                if (this.lastExecutedFunction === step.function) {
                    console.log(this.logger.bold(stepLine));
                } else {
     ...
     * @param {boolean} autoMode - Whether running in auto mode (skip key press)
     */
    async executeFunction(functionName, autoMode = false) {
        ...
        try {
            ...
            switch (functionName) {
                ...
                case 'cleanupNiFiOnly':
                    const NiFiCleanup = require('../implementation/cleanup-nifi');
                    const nifiCleanup = new NiFiCleanup();
                    result = await nifiCleanup.cleanupNiFiComponents();
                    break;
            ...
            // Always track last executed function for context
            this.lastExecutedFunction = functionName;
            ...
            // Skip in auto mode for true automation
            if (!autoMode) {
            ...
            // Track last executed function even on error
            this.lastExecutedFunction = functionName;
            // Skip key press in auto mode
            if (!autoMode) {
                await this.waitForKeyPress();
            }
            return false;
     ...
     * @param {object} section - The section to execute
     * @param {boolean} autoMode - Whether running in auto mode (affects error handling)
     */
    async executeSection(section, autoMode = false) {
        ...
        for (const step of section.steps) {
            if (!await this.executeFunction(step.function, true)) {  // Pass true for auto mode
                allSuccess = false;
        ...
        if (allSuccess) {
            ...
            if (autoMode) {
                this.logger.info('Auto mode: Returning to sections menu...');
                await new Promise(resolve => setTimeout(resolve, 1000)); // Brief pause
            }
        } else {
            this.logger.error(`${section.name} failed`);
            if (autoMode) {
                this.logger.info('Auto mode halted due to error');
                await this.waitForKeyPress();
            }
        }
     ...
     */
    async executeAllSections() {
        this.logger.header('Auto-executing all sections');
        
        if (!this.consoleConfig || !this.consoleConfig.sections) {
            this.logger.error('No sections configured');
            return false;
        }
        
        for (const section of this.consoleConfig.sections) {
            this.logger.newline();
            this.logger.header(`Starting section: ${section.name}`);
            
            const success = await this.executeSection(section, true); // Pass true for auto mode
            this.lastExecutedSection = section.key;
            
            if (!success) {
                this.logger.error('Section failed, stopping auto execution');
                // Error handling is already done in executeSection with waitForKeyPress
                return false;
            }
        }
        
        this.logger.newline();
        this.logger.success('All sections completed successfully!');
        this.logger.info('Returning to main menu...');
        await new Promise(resolve => setTimeout(resolve, 2000)); // Brief pause before returning
        return true;
    }
    ...
    async handleSection(sectionKey) {
        ...
        // Track that we entered this section
        this.lastExecutedSection = section.key;
        ...
        while (inSection && this.running) {
            ...
            switch (choice.toLowerCase()) {
                case 'a':
                    const sectionSuccess = await this.executeSection(section, true); // Pass true for auto mode
                    // Track last executed section
                    this.lastExecutedSection = section.key;
                    // If successful in auto mode, return to main menu
                    if (sectionSuccess) {
                        inSection = false; // Exit section menu and return to main
                    }
                    break;
                ...
                default:
                    ...
                    if (stepIndex >= 0 && stepIndex < section.steps.length) {
                        ...
                        // Track last executed section when a step is executed
                        this.lastExecutedSection = section.key;
                    } else {
