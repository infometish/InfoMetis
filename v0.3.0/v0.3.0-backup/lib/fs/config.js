...
class ConfigUtil {
    ...
    async loadImageConfig() {
        const configPath = this.resolvePath('v0.3.0/config/image-config.env');
        return await this.loadEnvConfig(configPath);
    }

    /**
     * Load console configuration for a version
     * @param {string} version - Version (e.g., 'v0.2.0', 'v0.3.0')
     * @returns {Promise<object>}
     */
    async loadConsoleConfig(version = 'v0.3.0') {
        const configPath = this.resolvePath(`${version}/config/console/console-config.json`);
