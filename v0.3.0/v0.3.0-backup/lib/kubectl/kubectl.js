...
class KubectlUtil {
    ...
    /**
     * Apply YAML manifest from file
     * @param {string} filePath - Path to YAML file
     * @param {string} description - Description for logging
     * @returns {Promise<boolean>}
     */
    async applyFile(filePath, description = 'manifest') {
        this.logger.step(`Applying ${description} from ${filePath}...`);
        
        const result = await this.exec.run(`kubectl apply -f ${filePath}`);
     ...
     * @param {string} resourceType - Resource type (e.g., 'statefulset', 'deployment')
     * @param {string} name - Resource name
     * @param {string} namespace - Namespace
     * @returns {Promise<boolean>}
     */
    async resourceExists(resourceType, name, namespace) {
        const result = await this.exec.run(
            `kubectl get ${resourceType} ${name} -n ${namespace}`, 
            {}, 
            true
        );
        return result.success;
     ...
     * @returns {Promise<boolean>}
     */
    async deleteResource(resourceType, name, namespace) {
        const result = await this.exec.run(
            `kubectl delete ${resourceType} ${name} -n ${namespace} --ignore-not-found=true`
        );
        return result.success;
    }
