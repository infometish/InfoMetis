...
class InteractiveConsole {
    ...
    async initialize() {
        try {
            ...
            this.consoleConfig = await this.config.loadConsoleConfig();
    ...
    async executeFunction(functionName, isAutoMode = false, stepName = null, sectionKey = null, stepIndex = null) {
        ...
        try {
            ...
            switch (functionName) {
                ...
                case 'saveContainersToTar':
                    result = await this.saveContainersToTar();
                    break;
                    
                case 'loadContainersFromTar':
                    result = await this.loadContainersFromTar();
                    break;
    ...
    async executeSection(section) {
        ...
        // For K section, exclude the last step (Remove Traefik) from auto execution
        const stepsToExecute = section.key === 'k' ? section.steps.length - 1 : section.steps.length;
        
        if (section.key === 'k' && stepsToExecute < section.steps.length) {
            this.logger.info(`ℹ️ Auto execution will skip the last step: "${section.steps[section.steps.length - 1].name}"`);
        }
        
        let allSuccess = true;
        for (let stepIndex = 0; stepIndex < stepsToExecute; stepIndex++) {
    ...
    /**
     * Save Docker containers to tar files
     */
    async saveContainersToTar() {
        this.logger.step('Saving Docker containers to tar files...');
        
        try {
            const result = await this.cache.saveToTar();
            if (result) {
                this.logger.success('All containers saved to tar files');
                return true;
            } else {
                this.logger.error('Some containers failed to save');
                return false;
            }
        } catch (error) {
            this.logger.error(`Container save failed: ${error.message}`);
            return false;
        }
    }

    /**
     * Load containers from tar files to Docker
     */
    async loadContainersFromTar() {
        this.logger.step('Loading containers from tar files to Docker...');
        
        try {
            const result = await this.cache.loadFromTar();
            if (result) {
                this.logger.success('All containers loaded from tar files');
                return true;
            } else {
                this.logger.error('Some containers failed to load');
                return false;
            }
        } catch (error) {
            this.logger.error(`Container load failed: ${error.message}`);
            return false;
        }
    }
