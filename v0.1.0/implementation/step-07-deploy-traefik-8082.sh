#!/bin/bash
# step-07-deploy-traefik-8082.sh
# Deploy Traefik with API on port 8082 (working version)
...
echo "📋 Creating Traefik Deployment with API on port 8082..."
docker exec infometis k0s kubectl apply -f - <<EOF
apiVersion: apps/v1
kind: Deployment
metadata:
...
echo "   API: http://localhost:8082/api/overview"
