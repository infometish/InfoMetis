...
echo "📋 Waiting for NiFi StatefulSet to be ready..."
docker exec infometis k0s kubectl wait --for=condition=ready --timeout=300s pod/nifi-0 -n "$NAMESPACE" || {
    echo "❌ NiFi pod not ready within timeout"
    echo "📋 Pod details:"
...
echo "✅ NiFi StatefulSet is ready"
